...
<div class="container py-4">
  ...
  <?php if (!$cestaAtiva): ?>
    <div class="alert alert-warning">
      Você não tem uma cesta ativa.
      <a href="/index.php?pagina=cestas" class="alert-link">Crie uma cesta</a> para continuar.
    </div>
  <?php else: ?>
...
function atualizarBotao() {
  ...
  document.querySelectorAll('#btn-adicionar-carrinho, #btn-adicionar-carrinho2').forEach(btn => {
  ...
  document.querySelectorAll('.chk-produto').forEach(chk => {
    ...
    const badge = document.getElementById(`badge-${chk.value}`);
    if (chk.checked) {
...
async function adicionarAoCarrinho() {
  ...
  if (!CESTA_ID) {
    alert('Crie uma cesta antes de adicionar produtos.');
    return;
  }
